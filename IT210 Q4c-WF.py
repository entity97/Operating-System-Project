...
###                         IT 210 Project                                          ###
###     Question 4  Memory Allocation Methods-Fixed Partition c) Worst Fit          ###
#######################################################################################
...
# and if the difference between the block size and the process size is greater than the current maximum difference. 
...
for i in range(pnum):
    max_diff = -1 
    # max_diff is initialized to -1 using the float() function. 
    # The purpose of this variable is to keep track of the maximum difference between a block size and a 
    # process size that has been found so far. This value is initialized to -1 so that the first difference 
    # that is found will always be larger than the current value of max_diff.
    ...
    for j in range(bnum):
        if bnlist[j] >= pnlist[i] and bnlist[j] - pnlist[i] > max_diff:
            max_diff = bnlist[j] - pnlist[i]
            best_block = j
